...
import { Mail, Phone, BookOpen, BookMinus, Download, Eye, Trash2, X, FileText, Calendar, User } from 'lucide-react';
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h2 className="text-2xl font-bold tracking-tight text-foreground">Career Submissions</h2>
          ...
          {submissions.map((submission) => (
            <div 
              ...
              className={`relative flex flex-col rounded-xl border p-6 shadow-sm transition-all hover:shadow-md ${
                submission.isRead ? 'bg-card border-border' : 'bg-card border-primary/50 ring-1 ring-primary/10'
              }`}
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className={`flex h-10 w-10 items-center justify-center rounded-full ${
                    submission.isRead ? 'bg-muted text-muted-foreground' : 'bg-primary/10 text-primary'
                  }`}>
                    <User size={20} />
                  </div>
                  <div>
        ...
        {submissions.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border p-12 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Career applications will appear here.</p>
          </div>
        )}
      </div>
          ...
          <div className="w-full max-w-lg rounded-xl bg-background shadow-2xl ring-1 ring-border animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              ...
              {viewingSubmission.resumeUrl && (
                <div className="rounded-lg border border-border bg-muted/50 p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Link 
                      href={viewingSubmission.resumeUrl} 
                      target="_blank" 
              ...
              {viewingSubmission.message && (
                <div>
                  <label className="text-xs font-medium uppercase text-muted-foreground">Cover Letter / Message</label>
      ...
      {deletingSubmissionId && (user?.role === 'ADMIN') && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-xl bg-background p-6 shadow-2xl ring-1 ring-border animate-in fade-in zoom-in-95 duration-200">
            <h3 className="text-lg font-semibold text-foreground mb-2">Delete Application</h3>
